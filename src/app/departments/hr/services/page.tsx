...
export default async function HRDepartmentServicesPage() {
  ...
  const logoUrl = loadSettingsServer().supportFunctionLogos.people

  return (
    <div className="space-y-6">
      <div className="rounded-xl border bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="flex h-14 w-14 items-center justify-center overflow-hidden rounded-xl bg-teal-600 text-white">{logoUrl ? <img src={logoUrl} alt="People Team" className="h-full w-full object-contain p-1.5" /> : <Users className="h-7 w-7" />}</div>
