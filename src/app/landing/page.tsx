...
const baseFunctions: LandingDestination[] = [
  ...
  },
  {
    ...
    icon: Headset,
    accent: "border-cyan-200 bg-cyan-50 text-[#263d8b]",
    status: "SolarWinds",
    external: true,
    ...
    <main className="relative min-h-screen bg-[#f4f8fd] px-4 py-5 text-slate-900 dark:bg-[#0b1220] dark:text-slate-100 lg:h-screen lg:overflow-hidden sm:px-6 sm:py-6">
      ...
      <div className="relative mx-auto w-full max-w-[1728px]">
        <header className="mb-5 flex flex-col items-center text-center sm:mb-6">
          ...
          <p className="mt-2 max-w-2xl text-[15px] leading-6 text-slate-600 sm:max-w-none sm:whitespace-nowrap dark:text-slate-400">
            Welcome {session.user.name || session.user.email}, Choose the team whose services you need.
          </p>
        </header>

        <section className="rounded-[28px] border border-blue-100 bg-gradient-to-br from-white/95 via-white/80 to-blue-50/75 p-5 shadow-[0_18px_60px_-38px_rgba(30,64,175,0.5)] backdrop-blur-sm dark:border-slate-800 dark:from-slate-900 dark:via-slate-900 dark:to-blue-950/40 sm:p-6" aria-labelledby="support-functions-heading">
          <div className="mb-5 text-center">
            ...
            <p className="mt-1.5 text-sm leading-5 text-slate-500">Choose a team to submit or manage requests.</p>
          </div>
          ...
          {functions.map((item) => {
            ...
            const content = (
              <>
                ...
                <p className="mt-1.5 text-sm leading-5 text-slate-600 dark:text-slate-400">{item.description}</p>
                <div className="mt-auto flex items-center pt-4 text-sm font-bold text-blue-600">
        ...
        {visibleMainApps.length > 0 && (
          <section className="mt-5 rounded-[24px] border border-slate-200/80 bg-white/35 p-4 shadow-[0_12px_45px_-35px_rgba(15,23,42,0.4)] dark:border-slate-800 dark:bg-slate-900/35" aria-labelledby="main-apps-heading">
            <div className="mb-3 text-center">
              ...
              <p className="mt-1.5 text-sm leading-5 text-slate-500">Open the applications you use every day.</p>
            </div>
            <div className={`grid min-w-0 max-w-full gap-3 overflow-hidden sm:grid-cols-2 lg:grid-cols-4 ${mainAppsGridColumns}`}>
              {visibleMainApps.map((item, index) => {
                ...
                const classes = "group flex min-h-[100px] min-w-0 items-center gap-3 rounded-2xl border border-slate-200 bg-white/90 p-4 shadow-sm transition duration-200 hover:-translate-y-0.5 hover:border-cyan-300 hover:bg-white hover:shadow-md dark:border-slate-800 dark:bg-slate-900 dark:hover:bg-slate-900"
