...
// Derived from canonical palette via STATUS_LABELS.
...
export default function PurchasePage() {
  ...
  const loadRequests = useCallback(() => {
    initializeMockData()
    const all = getRequests().filter((r) => r.module === "purchase")
  ...
  const allVisibleRequests = useMemo(() => {
    ...
    const allRequests = getRequests().filter((r) => r.module === "purchase")
      ...
      <Card>
        <CardHeader className="pb-4">
          ...
          <p className="text-sm text-muted-foreground font-normal mt-2">
            {showCcRequests
              ? `Showing ${filtered.length} order${filtered.length !== 1 ? "s" : ""} (including CC'd requests)`
