...
export default function EventPage() {
  ...
  useEffect(() => {
    initializeMockData()
    const sync = () => {
      let all = getRequests().filter((r) => r.module === "event")
        ...
        <div className="-mx-6 px-6 -mb-6 overflow-visible">
                ...
                <tr className={cn("border-b border-gray-100 hover:bg-blue-50/30 transition-colors", hasUnreadComments ? "bg-blue-50" : (i % 2 === 0 ? "bg-white" : "bg-gray-50/40"))}>
                  <td className="py-3 overflow-hidden" style={{ paddingLeft: 20, paddingRight: 8 }}>
                    <div className="flex items-center gap-2">
                      <Link href={`/requests/${req.id}?source=event`} className="text-sm font-medium text-blue-600 truncate block hover:underline">
                        {req.id}
                      </Link>
                  ...
                  <td className="py-3 px-3">
                    <span className="text-sm font-medium text-gray-700">
                      {(req.payload as Record<string, unknown>).eventDate
                        ? fmtDate(String((req.payload as Record<string, unknown>).eventDate))
                        : "—"}
                    </span>
                  </td>
                  <td className="py-3 px-3">
                    <span className="text-sm font-medium text-gray-700">
                      {String((req.payload as Record<string, unknown>).expectedAttendees ?? "—")}
                    </span>
                  </td>
                  <td className="py-3 px-3">
                    <InlineStatusSelect
                      ...
                      <div className="space-y-3 text-sm">
                        <div className="grid grid-cols-2 gap-6">
                          ...
                          <div>
                            ...
                            <p className="text-gray-600">{(req.payload as Record<string, unknown>).eventDate ? fmtDate(String((req.payload as Record<string, unknown>).eventDate)) : "—"}</p>
                          </div>
                          <div>
                            <p className="font-semibold text-gray-700">Attendees</p>
                            <p className="text-gray-600">{String((req.payload as Record<string, unknown>).expectedAttendees ?? "—")}</p>
                          </div>
                          <div>
          ...
          {filtered.length > 0 && (
            <div className="px-5 py-3 border-t border-gray-100 bg-gray-50/50 text-[11px] text-gray-400 text-right">
              Showing {filtered.length} of {companyRequests.length} events
            </div>
          )}
            </div>
          </div>
