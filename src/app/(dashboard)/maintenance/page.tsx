...
export default function MaintenancePage() {
  ...
  const loadRequests = useCallback(() => {
    initializeMockData()
    const all = getRequests().filter((r) => r.module === "maintenance")
  ...
  const allVisibleRequests = useMemo(() => {
    ...
    const allRequests = getRequests().filter((r) => r.module === "maintenance")
          ...
          <CardHeader className="pb-4">
            <div className="flex flex-wrap gap-3">
              <div className="relative flex-1 min-w-[200px]">
                ...
                <Input
                  placeholder="Search by ID, title, or comments…"
                  className="pl-9"
                  value={search}
            ...
            <p className="text-sm text-muted-foreground font-normal mt-2">
              {showCcRequests
                ? `Showing ${filtered.length} ticket${filtered.length !== 1 ? "s" : ""} (including CC'd requests)`
