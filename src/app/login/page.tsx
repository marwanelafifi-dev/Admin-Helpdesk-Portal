  ...
  useEffect(() => {
    ...
    if (authError) {
      const messages: Record<string, string> = {
        AccessDenied: "Google sign-in was denied. Check the allowed domain and use a verified company email.",
        OAuthCallback: "Google sign-in failed. Check that the Google client secret matches the client ID and that the redirect URI is allowed.",
        OAuthSignin: "Google sign-in could not start. Check the Google OAuth client configuration.",
        Configuration: "Authentication is not configured correctly. Check NEXTAUTH_URL, NEXTAUTH_SECRET, and Google OAuth settings.",
      }
      setError(messages[authError] ?? "Sign-in failed. Check the authentication settings and try again.")
    }
  }, [])
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        setError("Google sign-in could not start. Check the Google OAuth settings.")
      }
    } catch {
      setGoogleLoading(false)
      setError("Google sign-in could not start. Check the Google OAuth settings.")
